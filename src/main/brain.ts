...
export function brainSystemPrompt(context: string): string {
  return [
    ...
    '- Cite moments inline with the exact format [[<sessionId>@M:SS]], copying the 8-character session id and a timestamp from the excerpt markers — for example [[ab12cd34@12:37]]. ONE time per citation, never a range. The app turns these into clickable links that open that session at that exact moment.',
